...
namespace MergePDF
{
    public static class MergePDF
    {
        ...
        public static async Task Run([QueueTrigger("merge-pdf", Connection = "AzureWebJobsStorage")] string myQueueItem, ExecutionContext context, ILogger log)
        {
            ...
            if (config["IS_RUNNING_LOCALLY"] == "true")
            {
                log.LogInformation("Running locally... Getting Certificate from local settings.");
                certificate = new X509Certificate2(certBytes, password);
            }
            else
            {
                log.LogInformation("Running in Azure. Getting Certificate from keyvault.");
